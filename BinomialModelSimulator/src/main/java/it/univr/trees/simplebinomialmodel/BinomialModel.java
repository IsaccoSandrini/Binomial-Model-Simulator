 ...
 */
public class BinomialModel implements BinomialInterface{
	 ...
	 */
	public BinomialModel(double initialValue, double riskFreeRate, double sigma, int numberOfTimes, double finalTime) {
		...
		this.riskNeutralProbabilityUp = (Math.exp(riskFreeRate * finalTime/(double)(numberOfTimes))- downFactor)
				/ (upFactor - downFactor);
	...
	public double[][] getUpAndDownProbabilities() {
		...
		for(int i=0;i<numberOfTimes;i++) {
			probabilities[i][0]=riskNeutralProbabilityUp;
			probabilities[i][1]=riskNeutralProbabilityDown;			
		}
	...
	public double[] discount(double[] values, int timeIndex) {
		...
		for(int i=0; i < values.length; i++) {
			discountedValues[i] = values[i]/Math.exp((double)(timeIndex)*finalTime*riskFreeRate/(double)(numberOfTimes));
		}
